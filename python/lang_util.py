...
# 消息匹配模式
MSG_MATCH_CHK = r"^\s*[A-Za-z0-9_-]+\s*=\s*.+?(\s*#.*)?\s*$"  # 不含捕获组（检查是否有效消息）
# MSG_MATCH_G2 = r"^#?\s*([A-Za-z0-9_-]+)\s*=\s*(.+?)\s*$"  # 两个捕获组
# MSG_MATCH_G3 = r"^#?\s*([A-Za-z0-9_-]+)\s*=\s*(.*?)(\s#[A-Za-z0-9_-]+@\d+@\S*)\s*$"  # 三个捕获组
MSG_MATCH_G3 = r"^#?\s*([A-Za-z0-9_-]+)\s*=\s*(.*?)(\s#[A-Za-z0-9_-]+@\d+@\S*)?$"  # 三个捕获组(最后一个可以不存在)
